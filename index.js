...
import fusee_payload from "./compressed";
...
fusee_payload.length = 0;
  ...
  digitalToggle(led);
  try {
      device = webusb.requestDevice({ filters: [{ vendorId: 0x0955 }] });
  } catch (error) {
      console.log(error);
  ...
  let payload;
  payload = fusee_payload;

  if (launchPayload(payload)){
    pinMode(led, OUTPUT);
    digitalToggle(led);
  };